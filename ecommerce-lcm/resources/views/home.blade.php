                    ...
                    <div class="col-md-3 mb-4">
                        <div class="card h-100 shadow-sm">
                            ...
                            <div class="card-body d-flex flex-column">
                                ...
                                <p class="card-text text-success fw-bold">R$ {{ number_format($produto->preco ?? 0, 2, ',', '.') }}</p>
                                @if($produto->badge)
                                    <span class="badge bg-warning text-dark mb-2">{{ $produto->badge }}</span>
                                @endif
                                <form action="{{ route('carrinho.adicionar', $produto->id) }}, {{ route('carrinho') }}" method="POST" class="mt-auto">
                                    redirected to route
                                    @csrf
